...
堆不是 Top-K 的唯一答案，面试中能主动说出下面表会明显加分：

| 场景 | 堆 | 替代方案 | 原因 |
|------|------|---------|------|
| 静态数据 Top-K，K 小 | O(n log k) | **快选 QuickSelect** O(n) 期望 | 快选不需额外空间、期望复杂度更优；面试问 LC215 能主动提是加分点 |
| K 接近 n | O(n log n) | **直接排序** O(n log n) | 堆的优势在于 K 小，K≈n 时与排序同阶但常数更大 |
| 数据量极小（n ≤ 100） | O(n log k) | **直接排序** | 堆的常数项重，小数据反而慢；代码也多 |
| 需要第 K 大 + 所有顺序 | O(n log n) | **排序** | 堆只能拿到第 K 大，产生不了有序序列 |
| 范围查询 / 上下界 | — | **TreeMap / TreeSet** | 堆只能看堆顶，查任意范围要 O(n) |
| 需要从中间删除任意元素 | O(n) 扫描 | **TreeSet / 哈希+双链表** | 堆只支持 O(log n) 删堆顶，删中间元素要 O(n) 扫 |
| 多线程高吞吐优先队 | 需加锁 | **PriorityBlockingQueue / Disruptor** | `PriorityQueue` 不是线程安全，如果设计高吞吐调度器要提 |

**面试黄金句：**「堆适合 K 远小于 n 且数据动态到达的场景；K 接近 n 或静态数据用快选优于堆。」

### 为什么 LC215 面试要能同时讲出堆和快选？

- **堆 O(n log k)**：代码 5 行，思路简单，适合流式数据
- **快选 O(n) 期望**：原地 partition，期望复杂度更优，但最坏 O(n²)、代码长
- **两者都要会**，一般先写堆（不出错）再提快选（加分）
