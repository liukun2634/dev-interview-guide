...
| 专题 | 适用场景 | 典型题 |
|------|------|------|
| [双指针](./two-pointers) | 左右靠拢、快慢同步、原地修改 | 两数之和 II、删除重复项 |
| [滑动窗口](./sliding-window) | 最长/最短连续区间 | 最长无重复子串、最小覆盖子串 |
| [前缀和与差分](./prefix-sum-difference) | 区间和统计、批量区间更新 | 和为 K 的子数组、航班预订统计 |
| [二分查找](./binary-search) | 有序数组查找、边界定位 | 搜索旋转数组、查找首末位置 |
