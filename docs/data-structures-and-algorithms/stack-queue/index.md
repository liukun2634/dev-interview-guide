...
| 结构 | 不变式 | 典型场景 | 平均复杂度 |
|------|------|------|------|
| **单调栈（递增）** | 栈顶 = 当前最小；新值 ≤ 栈顶 则弹出 | 上一个更小元素、柱状图最大矩形 | O(n) 摄点（每个元素最多入出一次） |
| **单调栈（递减）** | 栈顶 = 当前最大；新值 ≥ 栈顶 则弹出 | 下一个更大元素、每日温度 | O(n) |
| **单调队列（递减）** | 队头 = 当前最大；从尾弹出小于新值的；从头弹出已出窗口 | 滑动窗口最大值 | O(n) |

**为什么是 O(n) 不是 O(n²)：** 表面上每个元素要与栈中多个元素比较，但**每个元素整个生命周期只入栈一次、出栈一次**，摄点后总操作仍是 O(n)。这是单调结构最关键的复杂度论证，面试必须会讲。
## 典型实例
...
| 专题 | 先看什么 |
|------|------|
| [电梯调度系统（工程）](./elevator-scheduling) | OOP + LOOK 调度算法 + 多梯成本函数派单——电梯 / 滑滅 / AGV / CPU 调度同一骨架 |
