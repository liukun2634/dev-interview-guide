...
**交换论证：为什么是结束时间而不是开始时间？**

这是面试官最爱追问的点。不背结论，能手推的简洁证明：

```
假设最优解是区间序列 OPT = [a₁, a₂, ..., aK]。
贪心解首选区间记为 g（结束最早）。

若 a₁ = g，贪心与 OPT 首选一致，递归中后续子问题 ✓
若 a₁ ≠ g：由于 g.end ≤ a₁.end（g 是结束最早），
         g 不会与 a₂...aK 重叠（因为 a₁ 也不重叠）。
         把 OPT 中的 a₁ 换成 g，得到同样 K 个区间的另一组最优解 ✓

结论：总存在一个最优解首选贪心选择，贪心正确。
```
...
```
区间 [0, 10], [2, 3], [4, 5]
按开始时间排序 → [0, 10] 在首位，选中后后面都被护住 → 只选 1 个
按结束时间排序 → [2, 3], [4, 5], [0, 10] → 选 2 个 ✓
```
...
| 题型 | 排序依据 | 理由 |
|------|------|------|
| 不重叠区间数 / 谈判安排 | 结束时间升序 | 给后续留最多空间 |
| 费类最少点个数 | 结束时间升序 | 同上 |
| 拼接最大数 / 最小数 | `(a+b)` vs `(b+a)` 字典序 | 全局最优不取决于单个大小而是拼接顺序 |
| 最少会议室 | 开始时间升序 + 堆 | 按开始时间处理，堆维护各会议室结束时间 |
