...
搜索"华为手机"时：查"华为"得 [Doc1, Doc3]，查"手机"得 [Doc1, Doc2]，取交集得 **Doc1**，再按相关性评分排序。
...
#### ES 写入路径（IndexBuffer → Segment → 持久化）

**ES 写入路径**是面试 Top 1 高频追问——能讲清 5 步，立刻显出深度。

```
1. 写请求 → IndexBuffer（内存）+ Translog（事务日志，落盘）
              ↓ refresh（默认 1s）
2. IndexBuffer → 内存 Segment（可搜索）
              ↓ 这一步后才能查到数据
3. flush（默认 30 分钟或 Translog 满 512MB）
              → 内存 Segment 写入磁盘 + 清空 Translog
4. Merge（后台）→ 小 Segment 合并为大 Segment
              → 删除被标记 deleted 的文档
```
...
| 操作 | 触发 | 作用 | 可搜索？|
|------|------|------|--------|
| **refresh** | 1s 自动 / `_refresh` API | IndexBuffer → 可搜索的内存 Segment | **是** |
| **flush** | 30min / Translog 满 | Segment 持久化 + 清 Translog | （已可搜）|
| **merge** | 后台 | 合并 Segment，回收 deleted 空间 | （已可搜）|

**性能调优**：批量导入时把 `refresh_interval=-1` + `replicas=0` → 速度提升 10× → 完成后再恢复。
...
| 规则 | 说明 |
|------|------|
| **Primary 数固定** | 创建索引时确定，**之后不能改**（除非 reindex）|
| **Replica 数可改** | `PUT /_settings { "number_of_replicas": 2 }` 随时调整 |
| **每个 Shard = Lucene Index** | 一个完整的倒排索引（有大小上限）|
| **Shard 大小建议** | **10-50 GB** 最佳（过小元数据爆炸，过大恢复慢）|
| **节点数 ≥ Replica 数 + 1** | 否则 Replica 会和 Primary 在同节点（失去冗余）|

#### 容量规划速算

```
Shard 数 = 数据总量 / 30GB (向上取整)
节点数 = (Shard 数 × (1 + Replica 数)) / 单节点能承受的 Shard 数 (通常 ≤ 600)

示例: 5TB 数据 + 1 副本
  Shard 数 = 5000 / 30 ≈ 170
  实际 Shard = 170 × 2 = 340 (含副本)
  最少节点 ≈ 340 / 50 ≈ 7 个数据节点（每节点 50 个 Shard 留余量）
```
...
::: tip 💡 BM25 比 TF-IDF 强在哪

> ① **TF 饱和**：词频从 1 增到 10 提升大，从 100 到 1000 几乎不增（避免堆砌关键词作弊）；② **长度归一化**：长文档容易词频高，所以按长度归一化（避免长文档霸榜）；③ **可调参数 k1、b**：业务可调权重曲线。

:::

#### 混合检索（Hybrid Search）—— 现代 ES 的杀手锏

**2024-2025 年 ES 8.x 标配**：稀疏检索（BM25）+ 稠密检索（向量）+ RRF 融合。**面试讲到现在还停留在"纯 BM25"已经落伍了**。
...
POST /products/_search
{
  "retriever": {
    "rrf": {
      "retrievers": [
        { "standard": { "query": { "match": { "title": "降噪耳机" }}}},
...
**RRF 公式**：`score = Σ 1 / (k + rank)`，k 默认 60。**不需要分数归一化**就能融合不同评分体系的结果。

#### 三种检索策略对比

| 维度 | 纯 BM25 | 纯向量 (kNN) | **Hybrid (BM25 + kNN + RRF)** |
|------|--------|------------|------------------------------|
| **精确关键词** | ✅ | ❌ | ✅ |
| **语义近似** | ❌（"耳机" 找不到 "headphone"）| ✅ | ✅ |
| **稀有词** | ✅ | ❌（向量训练不足）| ✅ |
| **延迟** | 低 | 中 | 中 |
| **召回率** | 中 | 中 | **高（互补）** |
| **生产推荐** | 旧系统 | 纯语义场景 | **2025 默认** |
...
2. 拼音/拼写纠错:
   "huawei" / "华为" / "华伟" 都能召回
3. 多字段加权:
   title^3 + content^1 + tags^2 (重要字段权重更高)
```
...
graph TB
    subgraph 离线训练
    direction TB
    H1[用户行为日志<br/>Hive / HDFS] --> H2[特征工程<br/>Spark]
    H2 --> H3[模型训练<br/>TensorFlow / PyTorch]
    H3 --> H4[模型产物<br/>Model Registry]
    end
    
    subgraph 在线服务
    direction TB
    U[用户请求] --> R1[召回层<br/>多路并行]
    R1 --> R2[粗排层<br/>轻量模型]
    R2 --> R3[精排层<br/>深度模型]
    R3 --> R4[重排层<br/>规则引擎]
    R4 --> F[返回推荐结果]
    ...
    subgraph 特征服务
    direction TB
    FS1[离线特征<br/>HDFS → Redis]
    FS2[近线特征<br/>Flink 聚合]
    FS3[实时特征<br/>Kafka → Redis]
    end
...
graph LR
    subgraph 推荐算法演进路线
    direction LR
    A["协同过滤<br/>UserCF / ItemCF<br/>简单有效"] --> B["矩阵分解<br/>ALS / SVD<br/>稀疏数据优化"]
    B --> C["深度学习<br/>Wide&Deep / DeepFM<br/>自动特征交叉"]
    C --> D["序列模型<br/>DIN / DIEN<br/>捕捉行为序列"]
    D --> E["大模型<br/>Embedding 召回<br/>多模态理解"]
    end
```

| 代际 | 代表算法 | 核心优势 | 局限 |
|------|----------|----------|------|
| 传统 CF | UserCF / ItemCF | 可解释性强，实现简单 | 稀疏矩阵、冷启动差 |
| 矩阵分解 | ALS / SVD | 处理稀疏数据，隐向量 | 无法利用丰富特征 |
| 深度学习 | Wide&Deep / DeepFM | 自动特征交叉，效果好 | 计算成本高 |
| 序列模型 | DIN / DIEN / SIM | 捕捉用户兴趣变化 | 训练数据量要求大 |
...
**分流策略**：
- 按 `userId % 100` 哈希分桶，不同桶分配不同实验组
- 保证同一用户在实验期间始终命中同一组（避免体验跳变）
- 支持多层实验（如召回层和排序层独立实验，互不干扰）

```mermaid
graph TB
    subgraph A/B 分流架构
    U[用户请求] --> H[哈希分桶<br/>userId % 100]
    H -->|桶 0-4| A[实验组 A<br/>新召回策略]
    H -->|桶 5-9| B[实验组 B<br/>新排序模型]
    H -->|桶 10-99| C[对照组<br/>线上基线]
    
    A --> M[指标采集]
    B --> M
    C --> M
    M --> D[统计分析<br/>t-test / 置信区间]
    D -->|显著优于基线| E[全量上线]
    D -->|无显著差异| F[继续观察/放弃]
    end
```
...
**核心指标体系**：
- **短期指标**：CTR（点击率）、完播率、互动率（点赞/评论/分享）
- **长期指标**：次日留存率、7日留存率、人均观看时长
- **护栏指标**（guardrail）：负向指标不能恶化，如举报率、退出率

**决策标准**：实验组在核心指标上统计显著（p-value < 0.05）优于对照组，且护栏指标不恶化，才能全量上线。
