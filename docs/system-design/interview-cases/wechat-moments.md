...
| 人均日发帖 | 约 | 2 条/天 |
| 平均好友数 | 约 | 150 人 |
| 写扩散 inbox 写入量 | 9亿 × 2 × 150 | **2,700 亿次/天** |
| 平均 inbox 写入 QPS | 2,700亿 ÷ 86,400 | **约 312 万次/s** |
| 峰值系数 | 6× | **约 1,872 万次/s** |
| 单 MySQL 分片写入上限 | | ~5 万次/s |
| 所需分片数 | 1,872 万 ÷ 5 万 | **约 374 个分片** |

这解释了为什么微信朋友圈的 Feed 存储需要数百个分片，也是超过 5000 好友的账号（大 V）**必须切换为读扩散**的根本原因——若按写扩散，单条帖子就产生 5,000 次 inbox 写入，瞬时形成热点写入压力。
...
> **时钟偏斜（Clock Skew）处理**：两条帖子在同一毫秒内由不同服务器创建时，`created_at` 相同，仅靠时间戳无法保证游标稳定性。解决方案：`post_id` 采用 Snowflake ID（高位为时间戳 + 机器 ID + 序列号），即使 `created_at` 相同，`post_id` 也是严格单调递增的，因此 `(created_at, post_id)` 复合游标始终稳定可排序。
