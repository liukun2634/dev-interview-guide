...
| 差异类型 | 说明 | 处理流程 |
|---------|------|---------|
| **内部有，银行无（短款）** | 我方记录了支付成功，银行无对应记录 | 1. 重新查询银行接口（trade_no 查询）<br>2. 确认银行未到账 → 挂起，等待 T+2 银行补发清算文件<br>3. T+2 仍无记录 → 人工工单 + 暂时冻结用户账户 |
| **银行有，内部无（长款）** | 银行记录了扣款，我方无对应订单 | 1. 查询内部所有状态订单（含失败/超时订单）<br>2. 找到匹配订单 → 更新状态为成功并入账<br>3. 未找到匹配 → **立即退款给用户**（不能占用用户资金） |
| **金额不一致** | 双方都有记录但金额不同 | 1. 以**银行金额为准**（银行是资金权威方）<br>2. 内部金额偏大 → 退差价给用户<br>3. 内部金额偏小 → 补充入账差额 |
| **状态不一致** | 内部显示处理中，银行已完成 | 1. 以银行最终状态为准更新内部状态<br>2. 触发后续业务流程（发货/服务开通） |

> **核心原则**：出现资金差异时，**宁可我方损失，不可让用户受损**。长款必须退还，短款需调查，金额差以银行为准。
...
支付系统中 ML 风控模型是拦截欺诈交易的核心防线，必须在不影响正常支付体验的前提下完成判定。

**特征工程（三类核心特征）：**

| 特征类别 | 示例 | 更新频率 |
|---------|------|---------|
| 速度特征（Velocity） | 同设备1分钟内支付次数、同账户1小时内收款次数 | 实时（Flink 滑动窗口） |
| 设备指纹 | 设备 ID、IP、GPS 位置、UA、操作系统版本 | 每次请求 |
| 图特征（Graph） | 收款账户的资金网络（洗钱路径检测） | T+1 离线图计算 |
...
- **同步模型（LightGBM）**：特征 150 个（速度特征 + 设备指纹），P99 < 50ms，在支付链路上同步调用；阈值调整：宁可误杀 1% 正常用户，不放过 0.01% 欺诈
- **异步模型（DNN/图神经网络）**：特征更丰富（含图特征），200ms 内完成，支付完成后异步调用；判定欺诈则事后冻结 + 短信通知
- **模型更新**：欺诈模式变化快（攻防对抗），LightGBM 每周重训，DNN 每月重训；新样本来自「已确认欺诈」的交易（T+2 对账确认）

> **LightGBM 与 DNN 并行执行架构：**  
> 两个模型**并行**运行（不是串行），各自有独立的 GPU 推理服务器：同步 LightGBM（CPU推理，P99 < 30ms）和异步 DNN（GPU推理，P99 < 200ms）同时接收支付事件。最终风控决策取两者**加权融合分数**（w₁×LGB_score + w₂×DNN_score），阈值由业务策略配置。LightGBM 超时（30ms）时降级为**规则引擎兜底**（速度特征简单阈值判断）。

**生产挑战：**
- 模型误杀率 0.1% 对于日均 1000 万笔交易 = 每天 10,000 笔正常交易被拦截，运营成本高，需要快速申诉通道
- 欺诈模式漂移（Concept Drift）：新型欺诈手法出现后 48 小时内需要更新模型，需要快速训练和上线流程
