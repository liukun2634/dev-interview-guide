...
输入文本: "如何部署大语言模型"
   ...
   └────┬────┘
        │
   ┌────▼───────────────────┐
   │ Transformer Encoder     │ ← 双向注意力，每个 Token 关注所有其他 Token
   │ (12~24 层)              │
   └────┬───────────────────┘
        │
   ┌────▼────┐
   │ Pooling  │ ← 将所有 Token 的表示聚合为一个向量
   └────┬────┘        （CLS token / Mean pooling / 加权平均）
        │
   [0.12, -0.45, 0.78, ...]  ← 最终 Embedding 向量（768~3072 维）
```
...
```
原始向量 (128维):
[0.12, -0.45, 0.78, ..., 0.33, -0.21, 0.56]
     ↓ 切分为 8 个子段（每段 16 维）
[segment1] [segment2] ... [segment8]
     ↓ 每个子段用码本中最近的中心替代
[code1=42] [code2=17] ... [code8=93]  ← 只需 8 个字节

存储对比:
  原始: 128 × 4 = 512 bytes
  PQ:   8 × 1 = 8 bytes         ← 压缩 64 倍
```
