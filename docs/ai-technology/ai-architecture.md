...
│
│  ┌─────────────────────┐
│  │ 多 Agent 协作        │ 多个 Agent 分工协作
│  ├─────────────────────┤
│  │ Agent Loop           │ 自主决策 + 工具调用循环
│  ├─────────────────────┤
│  │ RAG Pipeline         │ 检索增强生成
│  ├─────────────────────┤
│  │ Prompt Chain         │ 多步 Prompt 串联
│  ├─────────────────────┤
│  │ 单次 LLM 调用        │ 简单的输入→输出
│  └─────────────────────┘
│
...
```
无 KV Cache（每步重新计算所有 Token）:
  Step 1: 处理 [T1] ─── 计算 1 次注意力
  Step 2: 处理 [T1, T2] ─── 计算 2 次注意力
  Step 3: 处理 [T1, T2, T3] ─── 计算 3 次注意力
  总计: 1+2+3+...+n = O(n²)

有 KV Cache（只计算新 Token，复用缓存）:
  Step 1: 处理 [T1]，缓存 K1, V1
  Step 2: 处理 [T2]，用 Q2 与缓存 K1 做注意力
  Step 3: 处理 [T3]，用 Q3 与缓存 K1,K2 做注意力
  总计: 每步 O(n)，但只处理 1 个新 Token
```
