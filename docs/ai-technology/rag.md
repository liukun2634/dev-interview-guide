...
```
  ...
  │  原始文档          分块处理           向量化存储       │
  │  ┌──────┐     ┌──────────────┐   ┌───────────────┐  │
  │  │ PDF  │     │  Chunk 1     │   │ [0.12, 0.45,  │  │
  │  │ Word │ ──► │  Chunk 2     │──►│  0.78, ...]   │  │
  │  │ HTML │     │  Chunk 3     │   │ Vector DB     │  │
  │  │ ...  │     │  ...         │   │               │  │
  │  └──────┘     └──────────────┘   └───────┬───────┘  │
  │                                          │          │
  └──────────────────────────────────────────┼──────────┘
...
Embedding 模型将文本转换为稠密向量（Dense Vector），使得语义相近的文本在向量空间中距离更近。
...
- **中文场景**：优先选择 BGE 系列或 GTE 系列，中文语义捕获更精准
...
向量数据库用于存储和高效检索 Embedding 向量，是 RAG 系统的核心基础设施。
...
| **pgvector** | PostgreSQL 扩展 | 复用现有 PG 基础设施、支持混合查询 | 已有 PG 技术栈的团队 |
| **Qdrant** | 独立部署 | Rust 实现、性能优异、过滤灵活 | 高性能需求的生产环境 |
| **FAISS** | 库（非数据库） | Meta 开发、纯向量检索、不含持久化 | 研究实验、自建存储层 |

### 索引算法

向量数据库使用近似最近邻（ANN, Approximate Nearest Neighbor）算法加速检索：

- **HNSW（Hierarchical Navigable Small World）**：构建多层图结构，查询时从高层到低层导航，精度高但内存占用大
- **IVF（Inverted File Index）**：将向量空间聚类成多个区域，查询时只搜索相关区域，适合大规模数据
- **PQ（Product Quantization）**：将向量压缩编码，牺牲少量精度换取大幅节省内存
...
```
用户查询 ──► LLM 生成假设性回答 ──► Embedding ──► 向量检索
                                                     │
                                                     ▼
                                           检索到真实文档
```
